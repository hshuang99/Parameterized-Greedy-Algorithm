...
We established a direct correspondence between the bit-level expansion of the AES MixColumns matrix and the MixNibbles matrix of the KLEIN lightweight block cipher, both being $4 \times 4$ MDS matrices over $\mathbb{F}_{2^8}$ sharing the same defining polynomial $z^8 + z^4 + z^3 + z + 1$ (Chapter~\ref{ch:klein}). This correspondence allowed us to directly reuse our AES-optimized circuit for KLEIN's linear layer without re-solving the synthesis problem from scratch.
